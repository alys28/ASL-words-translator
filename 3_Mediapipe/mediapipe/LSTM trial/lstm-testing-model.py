...
def extract_coordinates(max_length):
    rows = []
...
def make_prediction(model_path, labels, csv_file, max_length):
    ...
    print(csv)
    coords = np.array(csv)
    coords = set_padding(coords, max_length)
    for frame in coords:
...
while True:
    ...
    print("LOADING...")
    time.sleep(0.5)
    print(make_prediction(model_path, labels, csv_file, max_length))
